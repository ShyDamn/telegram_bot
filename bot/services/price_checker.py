        ...
        while True:
            try:
                ...
                for user_id in users:
                    ...
                    # Собираем URL всех товаров пользователя
                    urls = [product['product_url'] for product in products if product.get('product_url')]
                    
                    if not urls:
                        continue
                        
                    # Получаем цены для всех товаров пользователя параллельно
                    prices = await self.parser.get_prices_batch(urls)
                    
                    # Проверяем цены и отправляем уведомления
